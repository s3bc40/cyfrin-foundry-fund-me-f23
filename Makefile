...
# build:; forge build # one liner
# # To deploy on sepolia but not enough fund for gas fees
# deploy-sepolia:
# 	forge script script/DeployFundMe.s.sol:DeployFundMe --rpc-url $(SEPOLIA_RPC_URL) --account metamaskAccount --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv

# From Makefile repo

.PHONY: all test clean deploy fund help install snapshot format anvil zktest # to tell makefile they are not folders
